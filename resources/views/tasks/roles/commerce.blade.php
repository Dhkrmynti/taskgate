...
@section('title', 'Manage Commerce Task')
...
<div class="space-y-3">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('commerce-rekon.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-brand-muted hover:text-brand-text transition dark:text-slate-400 dark:hover:text-white">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Kembali ke Dashboard Commerce
        </a>
        <div class="flex items-center gap-3">
            <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">Mode: Commerce Editor</span>
        </div>
    </div>
    ...
    <section class="rounded-xl border border-blue-500/20 bg-blue-50/10 p-4 dark:border-blue-900/30 dark:bg-blue-900/10">
        ...
        <p class="text-xs text-slate-500 font-mono mt-2">ID: {{ $project->id }}</p>
    </section>
    ...
    <section class="rounded-xl border border-[#e6e7f0] bg-white p-3 shadow-soft dark:border-brand-darkLine dark:bg-brand-darkPanel">
        <div class="mb-8">
            <h3 class="text-lg font-bold">Sub-Fase Progress: Commerce</h3>
